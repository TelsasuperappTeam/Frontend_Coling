...
const RiwayatTransaksiEM = () => {
  ...
  /**
   * FETCH TRANSAKSI (Khusus EM)
   * Token EM sudah terikat 1 kebun di Backend, jadi tidak perlu parameter target_kebun_auth_id
   */
const fetchSemuaTransaksi = async () => {
    ...
    try {
      ...
      // --- BUKTI UNTUK BACKEND (Log URL) ---
      console.log(`[DEBUG FE EM] Menembak API Transaksi Aktif: ${urlBase}?is_history=false`);
      console.log(`[DEBUG FE EM] Menembak API Transaksi Histori: ${urlBase}?is_history=true`);
      ...
      // --- BUKTI UNTUK BACKEND (Log Respon) ---
      console.log("[DEBUG FE EM] Respon GET Transaksi Aktif dari BE:", dataAktif);
      console.log("[DEBUG FE EM] Respon GET Transaksi Histori dari BE:", dataHistori);

      setAllData([...dataAktif, ...dataHistori]);
    } catch (error) {
      console.error("Error fetch transaksi EM:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 sm:p-8 md:p-10 min-h-screen font-sans text-gray-800 bg-white">
      {/* HEADER HERO */}
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs sm:text-sm mt-0.5">
              Pantau armada logistik, pendapatan, dan proses bagi hasil kebun Anda (Read-Only).
            </p>
          </div>
        </div>
      </div>
  ...
  if (statusPermintaan === "menunggu") {
    ...
    if (item.pemeriksaan) {
      if (!item.bagi_hasil) {
        ...
        label = "Menunggu Proses Bagi Hasil";
      } else {
    ...
    <MainCard>
      ...
      {isExpanded && (
        <div className="mt-5 sm:mt-8 pt-5 sm:pt-8 border-t border-gray-100 space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-top-2">
          ...
          {/* 2. BOX HIJAU: NOTA TIMBANGAN PABRIK */}
          {item.pemeriksaan && (
          ...
          {/* 3. BOX AKSI BAGI HASIL (READ-ONLY) */}
          {item.pemeriksaan && (
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm relative overflow-hidden">
                  ...
                  <div>
                    <h4 className="font-bold text-[#EF8523] text-sm sm:text-base flex items-center gap-2">
                      <AlertCircle className="w-5 h-5" /> Menunggu Pihak Kebun
                    </h4>
                    <p className="text-[11px] sm:text-xs text-orange-800 mt-1 max-w-md leading-relaxed">
                      Pabrik telah mengeluarkan harga final. Menunggu admin Kebun untuk melakukan distribusi bagi hasil ke petani.
                    </p>
                  </div>
...
export default RiwayatTransaksiEM;