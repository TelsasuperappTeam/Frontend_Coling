...
const DOKUMEN_CONFIG = [
  // --- Prinsip 2. Penerapan Praktik Pertanian yang Baik ---
  {
    ...
    deskripsi: "Upload dokumen berisi: Nama anggota, NIK, Lokasi lahan, dan Luas lahan.",
    code: "P2_2_1_ANGGOTA", 
  },
  {
    id: 2,
  ...
  },
  {
    id: 7,
    prinsip: "Prinsip 2. Penerapan Praktik Pertanian yang Baik",
    kriteria: "Kriteria 2.3 Penerapan Teknik Budidaya dan Pengangkutan Kelapa Sawit",
  ...
  {
    id: 18,
    prinsip: "Prinsip 3. Pengelolaan Lingkungan Hidup, SDA, dan Keanekaragaman Hayati",
  ...
  // --- Prinsip 4. Penerapan Transparansi ---
  {
    ...
    deskripsi: "Pantau grafik pergerakan harga TBS sesuai standar pemerintah di halaman utama dashboard Anda.",
    isRedirect: true,
...
const Operasional2 = () => {
  ...
  // 2. Fetch Pengurus
  const fetchDataPengurus = useCallback(async (kebunAuthId) => {
    if (!kebunAuthId) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[DEBUG FE] Respon Data Pengurus BE untuk Kebun ID ${kebunAuthId}:`, data);
        
        setPengurusList(Array.isArray(data) ? data : []);
      } else {
        // --- CONSOLE LOG JIKA ERROR DARI BE ---
        console.warn(`[DEBUG FE] Respon Pengurus Gagal. Status: ${response.status}`);
        setPengurusList([]);
      }
    } catch (error) {
    ...
    try {
      ...
      if (isGM) {
        ...
        if (res.ok) {
          ...
          const single = [{ auth_id: validId, nama_lengkap: data.nama_lengkap }];
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs sm:text-sm">
              Pantau dokumen organisasi dan legalitas operasional kebun (Read-Only).
            </p>
          </div>
        </div>
                ...
                return (
                  <div
                    ...
                    className={`px-5 py-3 text-sm cursor-pointer transition-colors flex items-center justify-between ${
                      isSelected ? "bg-red-50 text-[#B5302D] font-bold" : "text-gray-700 hover:bg-gray-50"
                    }`}
                  >
                    {namaKebun}
                    ...
                    {loadingPengurus ? (
                      ...
                      pengurusList.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="p-4 text-gray-500">{item.tugas_pengurus || "-"}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan="5" className="p-8 text-center text-gray-400 font-bold italic">
              ...
              <div className="-mt-4 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-6">
                <div className="max-w-3xl">
                  ...
                  <p className="text-[13px] text-gray-500 font-normal leading-relaxed">
                    Pantau kelengkapan dokumen standar Indonesian Sustainable Palm Oil (ISPO). Role Anda hanya dapat melihat (Read-Only).
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-12">
                {Array.from(new Set(combinedDocs.map((d) => d.prinsip))).map(
                  (namaPrinsip, prinsipIdx) => {
                    ...
                    const docsInPrinsip = combinedDocs.filter((d) => d.prinsip === namaPrinsip);
                                    ...
                                    return (
                                      <div
                                        ...
                                        className={`p-5 sm:p-6 flex flex-col md:flex-row md:items-start gap-5 transition-all duration-300 ${
                                          isUploaded && !isRedirectItem ? "bg-emerald-50/20" : "hover:bg-gray-50/80"
                                        }`}
                                      >
                                            ...
                                            <div className="pt-1.5">
                                              {isRedirectItem ? (
                                                ...
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold bg-blue-50 text-blue-500 border border-blue-100">
                                                  <Loader2 className="w-3 h-3 animate-spin" /> Memuat status...
                                                </span>
                                              ) : isUploaded ? (
                                        ...
                                        <div className="flex flex-col gap-2.5 md:w-40 mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-100 shrink-0">
                                          {isRedirectItem ? (
                                            <button
                                              onClick={() => navigate(doc.redirectPath)}
                                            ...
                                            <button
                                              onClick={() => window.open(getFileUrl(doc.file_url, "ISPO"), "_blank")}
