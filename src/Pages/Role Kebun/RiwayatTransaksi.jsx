...
const RiwayatTransaksi = () => {
  ...
  const fetchRiwayatSelesai = async () => {
    ...
    try {
      ...
      // Menggunakan endpoint pengajuan milik kebun
      const response = await fetch(API_ENDPOINTS.FARM.MARKETPLACE.GET_DAFTAR_PENGAJUAN_KEBUN, {
        method: "GET",
      ...
      if (response.ok) {
        // FILTER: Hanya ambil yang statusnya benar-benar selesai sesuai instruksi
    ...
    <div className="space-y-6 p-4 md:p-8 min-h-screen font-sans bg-gray-50/30">
      ...
      <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex gap-3">
        ...
        <p className="text-xs md:text-sm text-blue-800 leading-relaxed">
          Halaman ini menampilkan seluruh data transaksi yang telah <strong>selesai diproses oleh pabrik</strong>, termasuk hasil pemeriksaan akhir dan potongan.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          <div className="text-center py-10 text-gray-500 text-sm">Memuat riwayat...</div>
          ...
          riwayatSelesai.map((item) => (
            <div key={item.id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden transition-all">
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium">
                      {item.nama_grup} • {item.estimasi_total_tbs_grup_kg / 1000} Ton
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-5 border-t sm:border-t-0 pt-3 sm:pt-0">
                  <div className="text-left sm:text-right flex flex-col sm:items-end gap-1">
                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Status Akhir</p>
                  ...
                  <button className="py-1.5 px-4 border border-gray-200 rounded-lg text-[11px] font-bold text-gray-600 flex items-center gap-1.5">
                    ...
                    {openDetailId === item.id ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                  </button>
                </div>
              </div>
                ...
                <div className="border-t border-gray-100 bg-white animate-fadeIn">
                  <div className="p-5 bg-gray-50/50">
                    <h5 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <ClipboardList size={14} /> Rekapitulasi Riwayat Pemeriksaan
                    </h5>
                    
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {/* INFORMASI DASAR */}
                      <div className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
                        <label className="text-[10px] text-gray-400 font-bold block uppercase">Tanggal Kirim</label>
                        <p className="font-bold text-gray-800 text-sm">
                          {new Date(item.tanggal_rencana_panen).toLocaleDateString("id-ID")}
                        </p>
                      </div>
                      <div className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
                        <label className="text-[10px] text-gray-400 font-bold block uppercase">Jenis Sawit</label>
                        <p className="font-bold text-gray-800 text-sm">{item.jenis_varietas_gabungan}</p>
                      </div>
                      <div className="bg-white p-3 rounded-lg border border-green-100 shadow-sm">
                        <label className="text-[10px] text-green-600 font-bold block uppercase">Netto Akhir (Kg)</label>
                        <p className="font-black text-green-700 text-sm">
                          {(item.estimasi_total_tbs_grup_kg).toLocaleString("id-ID")} Kg
                        </p>
                      </div>
                      <div className="bg-white p-3 rounded-lg border border-red-100 shadow-sm">
                        <label className="text-[10px] text-red-500 font-bold block uppercase">Total Potongan</label>
